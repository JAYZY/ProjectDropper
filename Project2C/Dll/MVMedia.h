...
#ifdef CTMEDIA_EXPORTS
#define CTMEDIA_API __declspec(dllexport)
#else
#define CTMEDIA_API __declspec(dllimport)
#endif
...
#define CT_MAKEFOURCC(ch0, ch1, ch2, ch3)                              \
	...
	enum {
		...
		CT_FOURCC_MONO = CT_MAKEFOURCC('M', 'O', 'N', 'O'),
...
#define CT_MEDIA_ERROR_MOREDATA						(-10)
	...
	CTMEDIA_API void Roseek_Media_SetLogLevel(UINT32 Level);
	enum
	{
		MEDIA_SCALE_MODE_KEEP_VS_HS_RATIO,
		MEDIA_SCALE_MODE_NO_BLACK_BLANKING,
	};
	CTMEDIA_API void Roseek_MediaEncoder_SetScaleMode(UINT32 Mode);
