    ...
    class JpgCompress {
        ...
        public static CFviImage Decompress(byte[] jpg_buffer) {
           ...
           // var t1 = DateTime.Now;
            ...
            uint uImgBufferLen = (uint)jpg_buffer.Length;
            ...
            lock (obj) {
                /*获取jpg图片相关信息但并不解压缩*/
                int ret = tjDecompressHeader3(hDec, pBufferPtr, uImgBufferLen, out jpegWidth, out jpegHeight, out jpegSubsamp, out jpegColorspace);
                if (0 != ret) {
                    tjDestroy(hDec);
                    return null;
                }
            }
            JpgCompress.TJPixelFormats tJPixelFormats = JpgCompress.TJPixelFormats.TJPF_RGB;
            if (JpgCompress.TJColorSpaces.TJCS_GRAY == (JpgCompress.TJColorSpaces)jpegColorspace) {
                tJPixelFormats = JpgCompress.TJPixelFormats.TJPF_GRAY;
            }
            else if (JpgCompress.TJColorSpaces.TJCS_RGB == (JpgCompress.TJColorSpaces)jpegColorspace) {
                tJPixelFormats = JpgCompress.TJPixelFormats.TJPF_RGB;

            }
            else if (JpgCompress.TJColorSpaces.TJCS_YCbCr == (JpgCompress.TJColorSpaces)jpegColorspace) {
                tJPixelFormats = JpgCompress.TJPixelFormats.TJPF_BGR;
            }
            ...
            lock (obj) {
                int flags = 0;
                try {
                    JpgCompress.tjDecompress2(hDec, pBufferPtr, uImgBufferLen, cfImg.GetImageAdrs(0, 0, 0), jpegWidth, 0, jpegHeight, (int)JpgCompress.TJPixelFormats.TJPF_GRAY, flags);
